...
nonisolated final class AppCloner: Sendable {
    ...
    private func installLaunchShim(
        ...
        # look. Each published manifest points at a wrapper that restores the
        # profile environment before starting the host, since browsers spawn
        # hosts with the real HOME. Registrations made mid-session are picked
        # up on the clone's next launch.
        sync_native_hosts() {
            local profile_nmh="$HOME/Library/Application Support/$1/NativeMessagingHosts"
            local real_parent="$REAL_HOME/Library/Application Support/$1"
            [ -n "$REAL_HOME" ] || return 0
            [ -d "$profile_nmh" ] || return 0
            [ -d "$real_parent" ] || return 0
            /bin/mkdir -p "$real_parent/NativeMessagingHosts" "$PARALLELIZER_PROFILE_ROOT/native-hosts" 2>/dev/null
            local manifest host_path name wrapper
            for manifest in "$profile_nmh"/*.json(N); do
                name="${manifest:t:r}"
                host_path="$(/usr/bin/sed -n 's/.*"path" *: *"\\([^"]*\\)".*/\\1/p' "$manifest" | /usr/bin/head -1)"
                [ -n "$host_path" ] || continue
                wrapper="$PARALLELIZER_PROFILE_ROOT/native-hosts/$name"
                /bin/cat > "$wrapper" <<'PARALLELIZER_WRAPPER'
        #!/bin/zsh
        \(exports)
        PARALLELIZER_WRAPPER
                /usr/bin/printf 'exec "%s" "$@"\\n' "$host_path" >> "$wrapper"
                /bin/chmod 755 "$wrapper"
                /usr/bin/sed -E 's|"path" *: *"[^"]*"|"path": "'"$wrapper"'"|' "$manifest" > "$real_parent/NativeMessagingHosts/$name.json"
            done
        }
        for browser_dir in "Google/Chrome" "Google/Chrome Beta" "Google/Chrome Dev" "Google/Chrome Canary" "Google/Chrome for Testing" "Chromium" "Microsoft Edge" "BraveSoftware/Brave-Browser" "Vivaldi" "Mozilla"; do
            sync_native_hosts "$browser_dir"
        done
